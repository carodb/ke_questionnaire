                        ...
                        foreach ($answer->getCols() as $column) {
                            ...
                            if (!$row->isTitleLine()) {
                                switch ($column->getShortType()) {
                                    case 'Radiobutton':
                                        $answers[$answer->getShortType()][$answer->getUid()]['rows'][$row->getUid()][$column->getShortType()][$column->getUid()]['value'] = $resultAnswerRepository->countResultAnswersForRowAndValue($row, $column->getUid());
                                        break;
                                    case 'Checkbox':
                                        $answers[$answer->getShortType()][$answer->getUid()]['rows'][$row->getUid()][$column->getShortType()][$column->getUid()]['value'] = $resultAnswerRepository->countResultAnswersForRowAndCol($row, $column->getUid());
                                        break;
                                    case 'SingleInput':
                                    case 'SingleSelect':
                                        $rrows = $resultAnswerRepository->getResultAnswersForRowAndColRaw($row, $column->getUid());
