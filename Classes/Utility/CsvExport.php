...
class CsvExport
{
    ...
    protected function createRBHeader($plugin)
    {
                ...
                foreach ($question->getAnswers() as $answer) {
                    if ($answer->exportInCsv()) {
                        switch ($answer->getType()){
                            case 'Kennziffer\KeQuestionnaire\Domain\Model\AnswerType\MatrixHeader':                                
                                $rows = $answer->getRows($question);                                    
                                foreach ($answer->getCols() as $col){
                                    foreach ($rows as $row){
                                        $this->RBStruct[$question->getUid()][$row->getUid()][$col->getUid()] = 1;
                                        $qL[] = $question->getUid();
                                        $qL2[] = $this->text . $question->getTitle() . $this->text;
                                        $aL[] = $col->getUid();
                                        $aL2[] = $this->text . $col->getTitle().'/'.$row->getTitle() . $this->text;
                                    };
                                }
                                break;
                            default:
        ...
        //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($header, 'header');
    ...
    protected function createRBLines($plugin)
    {
        ...
        foreach ($this->resultsRaw as $result) {
            ...
            foreach ($this->RBStruct as $questionId => $answers) {
                //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($answers, 'answers');
                ...
                foreach ($rAnswers as $rA) {
                    if ($rA['q_uid'] == $questionId) {
                        ...
                        } elseif ($rA['a_coltype'] == 'Kennziffer\KeQuestionnaire\Domain\Model\AnswerType\Checkbox') {
                            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($this->RBStruct[$questionId][$rA['a_uid']], 'struct '.$questionId.'/'.$rA['a_uid']);
                            foreach ($this->RBStruct[$questionId] as $rowId => $cols){
                                foreach ($cols as $colId => $colMarker){
                                    if (!$exportAnswers[$rowId][$colId]) $exportAnswers[$rowId][$colId] = '';
                                }
                            }
                            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($rA, 'rA');                                                                        
                            foreach ($answers[$rA['a_uid']] as $row => $cols){                                
                                //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($cols, 'row '.$row);   
                                if ($row == $rA['ra_col']) $exportAnswers[$rA['a_uid']][$row] = $rA['ra_col'];
                                elseif (!$exportAnswers[$rA['a_uid']][$row]) $exportAnswers[$rA['a_uid']][$row] = '';
                            }                                                              
                        } elseif (is_array($answers[$rA['a_uid']])) {    
                            foreach ($answers[$rA['a_uid']] as $row => $cols){
                                //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($cols, 'row '.$row);     
                                $exportAnswers[$rA['a_uid']][$row] = $rA['ra_value'];
                            }                            
                            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($rA, 'rA');                                                                        
                            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($answers, 'answers');                                                    
                        }
                ...
                //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($exportAnswers, 'Eanswers '.$questionId);     
                foreach ($answers as $answerId => $aVal) {                    
                    //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($aVal, 'answer '.$answerId);                                    
                    if (is_array($exportAnswers[$answerId])) {
                        foreach ($exportAnswers[$answerId] as $row => $col){
                            $val = '';                        
                            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($row, 'col/answerId '.$col.'/'.$answerId);     
                            if ($col == $row) $val = $this->getSingleMarker();
                            else $val = '';
                            $rL[] = $val;
                        }
                    } else {      
            ...
            //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($rL, 'rL');                        
            $lines .= implode($this->separator, $rL) . $this->newline;
        }        
        //\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($this->RBStruct, 'Eanswers');                        
        //exit;
        return $lines;
    }
                ...
                foreach ($question->getAnswers() as $answer) {
                    if ($answer->exportInCsv()) {
                        ...
                        if ($answer->getShortType() == 'MatrixHeader' OR $answer->getShortType() == 'ExtendedMatrixHeader') {
                            $answerLine = $answer->getCsvLineValuesForMatrixHeader($this->results, $question, $options, $oldArray, $counter);
